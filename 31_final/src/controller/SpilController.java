...
//		gui.setEjer(1, liste.getNavn(2));
		...
		//hver tur
		while (true) {
			//stopper løkken hvis der kun er 1 spiller tilbage
			if (liste.getPlayerAmount() == 1) {
				gui.visBesked(liste.getNavn(tur) + " , Du vandt!");
				break;
			}
			
			//styrer hvad der sker hvis man er i fængsel
			...
			//en spillers tur hvis spilleren ikker er i fængsel
			if (liste.getFængsel(tur) == 0) {
			...
			//opdatere alle spilleres penge på gui
			for (int i = 0; i < liste.getPlayerAmount(); i++) {
				gui.visPengebeholdning(liste.getNavn(i), liste.getKroner(i));
			}

			//løkke hvor man kan købe og sælge huse, og købe grunde af andre spillere
			while (true) {
			...
			//sætter spilleren i fængsel hvis han har slået 2 ens for meget
			...
			if (rafle.getTerningSlag(0) == rafle.getTerningSlag(1) && liste.getFængsel(tur) == 0) {
				...
				//giver turen videre til næste spiller
				liste.setExtraTur(tur, 0);
				tur++;
				if (tur >= liste.getPlayerAmount()) {
					tur = 0;
	...
	public void tilføjSpiller() {
		...
		for (int i = 0; i < antalSpillere; i++) {
			while (true) {
				//sikrer at ingen hedder det samme
